...
mydt = np.dtype([('team', np.str_, 16), ('oppTeam', np.str_, 16), ('mean', np.float32), ('std', np.float32)])

team_names = [ 'ATL', 'BOS', 'CLE', 'NOH', 'CHI', 'DAL', 'DEN', 'GOS', 'HOU', 'LAC', 'LAL', 'MIA', 'MIL', 'MIN', 'NJN', 'NYK', 'ORL', 'IND', 'PHL', 'PHX', 'POR', 'SAC', 'SAN', 'SEA', 'TOR', 'UTH', 'VAN', 'WAS', 'DET', 'CHH' ]
#team_names = [ 'ATL', 'BOS', 'CLE' ]


scores_10_fold = []

for teams in team_names:
    # choose a team and an opponent team (refer the above list)
    team    = teams  # Atlanta Hawks
    #oppTeam = 'BOS'  # Sacramento Kings
    for item in team_names:
        if item != team:
            oppTeam = item  # Sacramento Kings
            ...
            for i in range(0,32):
                ...
                yr += 1
                with open('Data/gameLog_'+yr_str+'.json') as json_data:
            ...
            scores_10_fold.append((team, oppTeam, scores.mean(), scores.std()))
...
scores_10_fold = np.asarray(scores_10_fold, dtype=mydt)
np.savetxt("scores_10_fold.txt", scores_10_fold, delimiter=",", fmt=('%16s', '%16s', '%10.4f', '%10.4f'), header='team, oppTeam, mean_10_fold, std_10_fold')
